...
class ParticipanteModel {
    ...
    public function getAmigoNombre() {
    ...
    public function delete() {
     ...
     public function liberarRestricciones()
    {
        // Traducido: "Actualiza la tabla participante poniendo 'amigo' vacio 
        // DONDE el campo 'amigo' sea igual al código que quiero borrar"
        $consulta = $this->db->prepare('UPDATE PARTICIPANTE SET amigo = NULL WHERE amigo = ?');
        $consulta->bindParam(1, $this->codigo);
        $consulta->execute();
    }

    public function limpiarAmigo(){
        $sql = $this->db->prepare("UPDATE PARTICIPANTE SET amigo = NULL WHERE cod_sorteo = ?");
        $sql->bindParam(1, $this->cod_sorteo);
        $sql->execute();
    }
